...
export default function ProductManager() {
  ...
  const filtered = products.filter(p =>
    !filter || `${p.brand} ${p.name}`.toLowerCase().includes(filter.toLowerCase())
  );
  ...
  return (
    <>
      ...
      <button className="primary" style={{ marginTop: 6 }}
              onClick={() => setEditing({ name: '', brand: '', category: 'foundation' })}>
        + New product
      </button>
...
function ProductForm({ product, onSave, onCancel }) {
  ...
  async function addShade() {
    if (!form.id) return;
    if (!shadeName) return;
    await admin.addShade({ productId: form.id, shadeName, hexColor: hex.match(/^#[0-9A-Fa-f]{6}$/) ? hex : null });
    setShadeName(''); setHex('#');
  }
  ...
  return (
    <div className="entity-card" style={{ marginTop: 12 }}>
      ...
      <input value={form.sephora_url ?? ''} onChange={e => set('sephora_url', e.target.value)} />
      <label>Ulta URL</label>
      <input value={form.ulta_url ?? ''} onChange={e => set('ulta_url', e.target.value)} />

      {form.id && (
        <>
          <h2 style={{ marginTop: 12 }}>Add shade</h2>
          <input placeholder="Shade name" value={shadeName} onChange={e => setShadeName(e.target.value)} />
          <input placeholder="#RRGGBB" value={hex} onChange={e => setHex(e.target.value)} style={{ marginTop: 4 }} />
          <button className="ghost" style={{ marginTop: 6 }} onClick={addShade}>Add shade</button>
        </>
      )}
