...
export default function ShadeTwinCalculator() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: pairs } = await supabase
        .from('shade_twins')
        .select('creator_a_id, creator_b_id, confirmed')
        .eq('confirmed', true);
      setConfirmedPairs(
        new Set(
          (pairs ?? []).map((p) => `${p.creator_a_id}|${p.creator_b_id}`),
        ),
      );
    })();
  }, []);
  ...
  const suggestions = useMemo(() => {
    ...
    return creators
      ...
      .map((c) => ({
        creator: c,
        score: jaccard(a, tuplesFromProfile(c.shade_profile)),
      }))
      .filter((s) => s.score > 0)
      ...
      {active && (
        <>
          <div style={{ fontSize: 12, color: 'var(--muted)', margin: '8px 0' }}>
            {active.skin_tone_desc
              ? `Tone: ${active.skin_tone_desc}`
              : 'No tone description set'}
          </div>

          <h2>Suggestions ({suggestions.length})</h2>
          {suggestions.length === 0 && (
            <div style={{ color: 'var(--muted)', fontSize: 12 }}>
              No overlapping tuples. Confirm more shades on creator profiles
              first.
            </div>
          )}
          {suggestions.map(({ creator, score }) => {
            ...
            return (
              <div key={creator.id} className="row">
                ...
                <button
                  className={isConfirmed ? 'ghost' : 'primary'}
