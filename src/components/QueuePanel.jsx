...
export default function QueuePanel() {
  ...
  async function handleCapture() {
    ...
    try {
      ...
      if (!result)
        setCaptureMsg('no video on screen — scroll to one and try again');
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from('tiktok_videos')
        .select(
          'id, video_url, creator_handle, caption, status, error, captured_at, nlp_result',
        )
        ...
        (payload) => {
          setVideos((prev) => {
            ...
            const idx = next.findIndex(
              (v) => v.id === payload.new?.id || v.id === payload.old?.id,
            );
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
        ...
        <button className="ghost" onClick={() => admin.openTikTokDevTools?.()}>
          TikTok DevTools
        </button>
      </div>
      {captureMsg && (
      ...
      {active.length === 0 && (
        <div style={{ color: 'var(--muted)', fontSize: 12 }}>
          Browse TikTok in the left panel — captured videos will appear here.
        </div>
      )}
      {active.map((v) => (
        ...
        >
          ...
          {audienceCount > 0 && (
            <span style={{ fontSize: 10, color: 'var(--muted)' }}>
              {audienceCount} audience signals
            </span>
          )}
        </div>
