...
import ManualShadeReport from './components/ManualShadeReport.jsx';
import { admin } from './lib/api.js';

const TABS = [
  { id: 'manual-report', label: 'Shade reports', Component: ManualShadeReport },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const offErr = admin.on('queue:error', (entry) => {
      setLogs((prev) => [
        ...prev.slice(-100),
        { level: 'error', msg: entry.error, ts: Date.now() },
      ]);
    });
    return () => {
      offLog?.();
